...
const sortIcicleDescending = (node: IcicleData) => {
  ...
  // Defensive: drop any accidentally-undefined/null entries before sorting
  // (e.g. from a bug that pushes a missing array element into children).
  node.children = node.children.filter((c): c is IcicleData => Boolean(c));
...
const transformBanDataForIcicle = (
  ...
  const bo1FirstAAll: MapStats[] = bo1FirstBanTeamAData
    ? [...bo1FirstBanTeamAData]
    : [];
  const bo1FirstBAll: MapStats[] = bo1FirstBanTeamBData
  ...
  bannedAll.forEach((map) => {
    ...
    const hasFirst = firstAll.find(
      (b) => b.map_name === map.map_name && b.count > 0,
    );
    const hasSecond = secondAll.find(
      (b) => b.map_name === map.map_name && b.count > 0,
    );
    const hasThird = thirdAll.find(
      (b) => b.map_name === map.map_name && b.count > 0,
    );
    ...
    const secondBanEntry = secondAll.find(
      (ban) => ban.map_name === map.map_name,
    );
    ...
    if (firstBanEntry && firstBanEntry.count > 0) {
      ...
      if (firstChildren.length >= 1) {
        const totalFirst = firstChildren.reduce(
          (s, c) => s + (c.value || 0),
          0,
        );
...
const CreateIcicleGraph: React.FC<CreateIcicleGraphProps> = ({
  ...
  useEffect(() => {
    if (innerRef.current)
      innerRef.current.style.width = innerWidth ? `${innerWidth}px` : "100%";
  }, [innerWidth]);
  ...
  const clampFocusScale = (v: number) =>
    Math.max(MIN_FOCUS_SCALE, Math.min(v, MAX_FOCUS_SCALE));
  ...
  const cloneAndScaleForMap = (
    root: IcicleData,
    targetName: string | null,
    scale: number,
  ): IcicleData => {
    if (!root || !root.children) return root;
    const cloneRoot: IcicleData = {
      ...root,
      children: root.children.map((c) => ({ ...c })),
    };
  ...
  useEffect(() => {
    ...
    const handler = (ev: WheelEvent) => {
      ...
      const totalUnits =
        root.children.reduce((s, c) => s + computeNodeValue(c), 0) || 1;
      ...
      const currentScale = innerRef.current
        ? innerRef.current.clientWidth / base
        : 1;
      ...
      const topTotal = topNode.children
        ? topNode.children.reduce((s, ch) => s + computeNodeValue(ch), 0)
        : 0;
      ...
      if (topNode.children && topNode.children.length > 0 && topTotal > 0) {
        ...
        if (leafIndex !== null) {
          ...
          setPathScales((prev) => ({
            ...prev,
            [key]: clampFocusScale(((prev[key] as number) || 1) * factor),
          }));
      ...
      setMapScales((prev) => ({
        ...prev,
        [target.name]: clampFocusScale(
          ((prev[target.name] as number) || 1) * factor,
        ),
      }));
                ...
                >
                  ...
                  {node.data?.name ? (
                    <>
                        ...
                        node.data.label.toString().includes("A - ") ? (
                          <>
                            <span style={{ color: "#ccc" }}>
                              ...
                              1st {WHATTYPEOTHER} of the match{" "}
                            </span>{" "}
                            <span
                          ...
                          <>
                            <span style={{ color: "#ccc" }}>
                              This team 1st {WHATTYPE} {node.data?.name} as the
                              2nd {WHATTYPEOTHER} of the match{" "}
                            </span>{" "}
                            <span
                              style={{ color: "#4CAF50", fontWeight: "bold" }}
                            >
                      ...
                      <>
                        {node.hierarchy.pathComponents.length == 2 ? null : (
                          <div>
                            <span style={{ color: "#ccc" }}>
