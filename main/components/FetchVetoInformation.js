   ...
   // console.log("penis 123",match);
    ...
    if (match.status === "finished") {
      ...
      if(!stats.skipPrinting || shouldFinish){
        ...
        if(stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[d].status == "pick"){
            amtOfPicks++;
        }
       // console.log("SELECTED BY??!? ",stats.PicksAndBans["payload"].tickets[2].entities[d].selected_by);
        if (stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[d].selected_by === "faction1") {
          stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[d].selected_by = fac1id;
        } 
        else if (stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[d].selected_by === "faction2"){
          stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[d].selected_by = fac2id;
        }
      }


      
        stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities.length-1].selected_by = stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities[stats.PicksAndBans["payload"].tickets[stats.PicksAndBans["payload"].tickets.length-1].entities.length-2].selected_by
