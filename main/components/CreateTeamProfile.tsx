...
function addRosterForSeason(memberz: any, season: any) {
  ...
  rosterCard.className =
    "flex gap-3 items-center border rounded-lg bg-cumground mt-1 -ml-1 flex-shrink-0 min-w-max mr-2";
  ...
  sortedMembers.forEach((member1: any, index: number) => {
    ...
    nameP.className =
      "text text-zinc-200 text-white [text-shadow:0px_1px_2px_black]";
    ...
    if (member1.count <= MostMatchesPlayed / 2) {
      avatarDiv.className =
        "w-10 h-10 rounded-full cursor-pointer hover:shadow-[0_0_10px_1px_white] transition duration-200";
    } else {
      avatarDiv.className =
        "w-12 h-12 rounded-full cursor-pointer hover:shadow-[0_0_10px_1px_white] transition duration-200";
    }
    avatarDiv.onclick = () =>
      window.open(
        "https://www.faceit.com/en/players/" + member1?.latest_player_name,
      );
    ...
    if (member1.count <= MostMatchesPlayed / 2) {
      avatarImg.className =
        "w-10 h-10 rounded-full hover:shadow-[0_0_8px_white]";
    } else {
      avatarImg.className =
        "w-12 h-12 rounded-full hover:shadow-[0_0_8px_white]";
    }
...
function sortRostersBySeason() {
  ...
  const rosterCards = Array.from(container.children).filter(
    (child) => child.id && child.id.startsWith("roster-"),
  ) as HTMLElement[];
...
export default function CreateTeamProfile() {
  ...
  const isMatchId =
    /^\d+-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/.test(user);
  ...
  const [loadingSeasonKeys, setLoadingSeasonKeys] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    if (data?.leagues?.[0]?.league_seasons_info && selectedKeys.size === 0) {
      ...
      const trySelectSeasonAndDivision = (
        seasonNumber: number,
        division: string,
      ) => {
      ...
      if (
        !trySelectSeasonAndDivision(
          primary,
          seasonObj.season_standings[0][0].division_name,
        )
      ) {
        // Try previous season as fallback
        trySelectSeasonAndDivision(
          primary,
          seasonObj.season_standings[0][1].division_name,
        );
      }
    }
  }, [data, selectedKeys.size]);

  // Swap the search input placeholder between team name and team id every 5 seconds.
  // Pauses while the input is focused so it doesn't change while the user types.
  useEffect(() => {
    const teamName = data?.teamdata?.name ?? "";
    const teamId = data?.teamdata?.team_id ?? "";
    const input = document.getElementById("searchText") as HTMLInputElement | null;
    if (!input || (!teamName && !teamId)) return;
    ...
    // Start with the team name as placeholder
    input.placeholder = teamName;

    const interval = setInterval(() => {
      if (focused) return; // don't swap while the user is interacting
      input.placeholder = showName ? teamId : teamName;
      showName = !showName;
    }, 5000);
  ...
  }, [data?.teamdata?.name, data?.teamdata?.team_id]);

  const OpenTeamName = (event: React.MouseEvent<HTMLParagraphElement>) => {
    event.preventDefault(); // if needed
    event.stopPropagation(); // if needed
    window.open(
      "https://www.faceit.com/en/teams/" + data.teamdata?.team_id + "/leagues",
    );
    // Your custom logic here
  };
  ...
  const handleSelectionChange = async (keys: any) => {
    ...
    const removed = prevArr.filter(
      (k: string) => !(keys as Set<string>).has(k),
    );
    ...
    if (B4Size - curSize < 0) {
      ...
      for (const season of data.leagues[0].league_seasons_info) {
        ...
        if (
          parseInt(season.season_number) ===
          parseInt(keys.currentKey.substring(1, 3))
        ) {
          let GotAllMyShit = [];
                ...
                if (team.team_id === data.teamdata?.team_id) {
                  ...
                  for (const player of team.players) {
                    ...
                    if (plyrObj) {
                      ...
                      if (
                        game.teamMatchData.teams.faction1.faction_id ===
                        data.teamdata?.team_id
                      ) {
                        for (const eachPlayerinTeam of game.teamMatchData.teams
                          .faction1.roster) {
                      ...
                      const latest = await FetchLatestUsername(
                        player.player_id,
                      );
                      ...
                      PlayedPlayers.push({
                        ...
                        latest_player_name: latest?.nickname,
                        avatar_img: latest?.avatar,
                        count: 1,
                      });
                    }
                  }
      ...
      for (const eachthinginlist of lastList) {
        ...
        if (!found) {
          ...
          for (const eachMatch of TeamData) {
            ...
            if (
              !(
                eachMatch.matchData.seasonNum ==
                  eachthinginlist.substring(1, 3) &&
      ...
      setUiNode((prev) =>
        prev.filter((node) => !removedIds.includes(node.props.id)),
      );
    } else {
  ...
  return (
    <div key={"TeamProfileCard"} className="w-full flex justify-center">
      <div className="relative w-full flex justify-center items-center min-h-[360px]">
        <div className="absolute top-0 w-full flex justify-center items-center -mt-13.5">
          <SearchText />
        </div>
        {loading ? (
          <div className="w-[1830px] ">
            <CreateSkeleton />
          </div>
        ) : (
          <div className="p-4 border rounded-xl shadow bg-cumground flex flex-col w-[2000px] mt-4">
            {/* team info */}
            <div className="flex gap-3">
              <div onClick={OpenTeamName} className="-mt-1">
                <Avatar
                  alt="Team1pfp"
                  size={150}
                  src={
              ...
              <div className="">
                <div className="flex ">
                  <div className="text-5xl mr-2 -mt-1 ">
                    <Flag
                      countryCode={
                        data.leagues[0].league_seasons_info[0].season_standings[
                          "0"
                        ][0].region_name
                      }
                    />
                                    ...
                                    {Array.from(
                                      ...
                                      .map((seasonNum) => {
                                        ...
                                        const isLoading = Array.from(
                                          ...
                                          .filter((key) =>
                                            key.startsWith(`S${seasonNum}`),
                                          )
                                          .some((key) =>
                                            loadingSeasonKeys.has(key),
                                          );
                                ...
                                (league: any) => {
                                  ...
                                  if (standing.length >= 1) {
                                    ...
                                    for (const eachDivision of standing) {
                                      ...
                                      if (
                                        eachDivision["1"] &&
                                        eachDivision["0"].division_name ===
                                          eachDivision["1"].division_name
                                      ) {
                                        const itemKey = `${baseKey} ${eachDivision["0"].division_name}`;
                                        const isLoading =
                                          loadingSeasonKeys.has(itemKey);
                                        ...
                                        items.push(
                                          <DropdownItem
                                              ...
                                              isLoading ? (
                                                ...
                                                <div className="mr-2 flex items-center">
                                                  ...
                                                  >
                                                    <path
                                                      clipRule="evenodd"
                                                      d="M16.704 5.29a1 1 0 010 1.42l-7.14 7.14a1 1 0 01-1.415 0l-3.06-3.06a1 1 0 111.415-1.415l2.353 2.353 6.433-6.433a1 1 0 011.415 0z"
                                                      fillRule="evenodd"
                                                    />
                                                  </svg>
                                                </div>
                                            ...
                                            startContent={
                                              <div
                                                className={
                                                  "min-w-[30px] flex items-center justify-center relative"
                                                }
                                              >
                                                <Image
                                                  className={
                                        ...
                                        const divisionItems = eachDivision.map(
                                          (eachdivisions: any) => {
                ...
                <div
                  className="flex flex-row overflow-x-auto w-full max-w-[98rem]"
                  id="PoopEater"
                >
            ...
            <div className="mt-2 flex">
                    ...
                    {needsPlaceholder ? (
                      ...
                      >
                        <p className="text-center text-lg text-white ">
                          Choose an ESEA season above in order to obtain
                          matches!
                        </p>
                      </Card>
                    ) : (
              ...
              <div className="ml-1 flex-1" id="FartSniffer">
                <CreateStatsPage
